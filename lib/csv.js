  ...
  return new Promise((resolve, reject) => {
    ...
    csv
      ...
      .on('data', async (data) => {
        ...
        const teamLogin = slug.substring(0, MAX_LOGIN_LENGTH) + Math.floor(Math.random() * 90 + 10) // randomized team login with min length of (5 + 2)
        ...
        try {
            const uid = await db.createUser(teamLogin, password, division, affiliation, teamName, emails)
            console.log(`${teamLogin}:${uid} imported!`)
        } catch(e) {
            console.log(`${teamName} import error. Please manually create the account.`)
        }
      })
      .on('end', () => {
